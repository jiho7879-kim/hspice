"""
corner_retrain_test_noise.py — Approach 3: Maximized noise contrast

Heteroscedastic noise contrast를 극대화하여 GP가 corner data에 강제로 fit:
  - FixedNoiseGaussianLikelihood with learn_additional_noise=False
  - Corner noise STD = 1e-6 (extremely low -> GP must pass through)
  - TT noise STD = 5e-2 (very high -> GP can deviate freely)
  
이렇게 하면 GP가 corner points를 거의 정확히 통과하면서도
TT data의 global trend는 유지.

Usage:
    cd python
    python scripts/corner_retrain_test_noise.py

Output:
    results/corner_retrain_noise/
"""
...
OUT_DIR       = Path(__file__).resolve().parent.parent / "results" / "corner_retrain_noise"
...
TT_NOISE_STD = 5e-2      # very high -> GP free to deviate
CORNER_NOISE_STD = 1e-6  # extremely low -> GP forced through

print("=" * 70)
print(f"Corner Retrain [3/4] - Max noise contrast")
print(f"  TT noise STD={TT_NOISE_STD}, Corner noise STD={CORNER_NOISE_STD}")
print(f"  learn_additional_noise=False")
print("=" * 70)
...
# Noise array: [N_tt + N_corner, 2]
y_noise_comb = np.zeros((len(X_comb), 2), dtype=np.float64)
y_noise_comb[:len(X_tt), :] = TT_NOISE_STD
y_noise_comb[len(X_tt):, :] = CORNER_NOISE_STD

print(f"\nNoise contrast ratio: {TT_NOISE_STD / CORNER_NOISE_STD:.0f}x")

# ---------------------------------------------------------------
# Custom Surrogate subclass: disable learn_additional_noise
# ---------------------------------------------------------------
class FixedNoiseSurrogate(Surrogate):
    """Same as Surrogate but FixedNoiseGaussianLikelihood with learn_additional_noise=False."""
    ...
    def _make_likelihood(self, col: int):
        ...
        return FixedNoiseGaussianLikelihood(
            noise=noise_var, learn_additional_noise=False,
        )

# Train
print("\nTraining with max noise contrast ...")
surr_new = FixedNoiseSurrogate(device="cpu")
...
for col, cn in enumerate(CORNER_NAMES):
    ...
    axes[0, col].plot(VOPS, r["mu_new"]*1000, "o-", color=colors[cn], label="Noise-contrast", markersize=5)
    ...
    axes[1, col].plot(VOPS, r["sigma_new"]*1000, "o-", color=colors[cn], label="Noise-contrast", markersize=5)
...
fig.suptitle(f"Max Noise Contrast (TT={TT_NOISE_STD}, corner={CORNER_NOISE_STD}, learn_additional_noise=False)", fontsize=12, y=1.01)
...
ax.bar(x+w, new_v, w, label="Noise-contrast GP", color="seagreen", alpha=0.85)
ax.set_xticks(x); ax.set_xticklabels(CORNER_NAMES)
ax.set_ylabel("Vmin (V)"); ax.set_title("Vmin Comparison (Max Noise Contrast)")
