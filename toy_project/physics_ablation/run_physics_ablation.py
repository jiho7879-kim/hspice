...
Compares 5 configurations:
    ...
    2. mono:      GP + L_mono (Vop↑→mu↑)
    3. boundary:  GP + corner anchor data augmentation
    4. mono+bnd:  GP + L_mono + corner anchors
    5. all:       GP + L_mono + corner anchors + L_pelgrom + lengthscale prior

Metrics:
    - mu/sigma RMSE & R² on hold-out test set
    - Vmin RMSE on hold-out test set
    - Contour Hausdorff distance (Vmin=0.6V boundary)
    - Area overlap ratio of feasible region
    - Gradient direction check (∂Vmin/∂cn, ∂Vmin/∂pu)
    - Lengthscale analysis (PG >> PU check)

Output:
    physics_ablation/results/
        contour_comparison.png — 5-panel contour overlay
        metrics_comparison.png — grouped bar chart
        gradient_table.txt     — gradient fidelity
        ablation_results.json  — all metrics in machine-readable format
"""
...
from src.utils import (
    ...
    load_intermediate, stratified_train_test_split,
)
from src.toy_surrogate import Surrogate
from src.toy_physics_layer import (
    compute_vmin_from_z, compute_vmin_on_grid, compute_zscore,
)
from src.toy_contour import extract_contour, hausdorff_distance, area_overlap_ratio

from physics_ablation.src.physics_constrained_surrogate import (
...
# Training
N_COND = 400          # number of (common_N, PU) conditions
N_ITER = 120           # GP training iterations (150 full, 120 for ablation speed)
LR = 0.1
...
# ===================================================================
# True Vmin from analytic model (ground truth for contour)
# ===================================================================
...
def compute_true_vmin_at_points(cn_vals: np.ndarray, pu_vals: np.ndarray) -> np.ndarray:
    """Compute true Vmin at specific (common_N, PU) points."""
...
def train_and_evaluate(
    ...
    vmin_rmse = float(np.sqrt(np.mean(vmin_errors ** 2))) * 1000  # mV
    vmin_max_err = float(np.max(np.abs(vmin_errors))) * 1000  # mV
    vmin_bias = float(np.mean(vmin_errors)) * 1000  # mV
    ...
    # True gradient from analytic model at (0, 0)
    ...
    # Cosine similarity with true gradient
    ...
    print(f"\n--- {cfg['label']} Results ---")
    print(f"  mu RMSE={mu_rmse:.6f}  R²={mu_r2:.4f}")
    print(f"  sigma RMSE={sigma_rmse:.6f}  R²={sigma_r2:.4f}")
    ...
    print(f"  mu lengthscales: cn={mu_ls[0]:.3f} pu={mu_ls[1]:.3f} Vop={mu_ls[2]:.3f}")
...
def plot_contour_comparison(
    ...
    # Hide last subplot if 5 configs
...
def plot_metrics_comparison(
    ...
    # Metrics to compare (lower is better, except R² and overlap)
    metric_defs = [
        ...
        ("mu_r2", "mu R²", 1, False, "{:.3f}"),
        ("sigma_r2", "sigma R²", 1, False, "{:.3f}"),
    ...
    for ax_i, (key, label, scale, lower_better, fmt) in enumerate(metric_defs):
        ...
        bars = ax.bar(x, [v * scale for v in values], width=bar_width,
        ...
        ax.set_title(f"{label} {'↓' if lower_better else '↑'}",
...
def plot_error_map(
    ...
    """Error maps (Vmin_pred - Vmin_true) for all configurations."""
    fig, axes = plt.subplots(2, 3, figsize=(18, 11))
    axes_flat = axes.flatten()
...
def print_gradient_table(all_metrics: dict) -> str:
    """Format gradient check comparison as text table."""
    lines = []
    lines.append("=" * 80)
    ...
    lines.append("=" * 80)
    lines.append("""Physical direction check:
  common_N +10mV -> Vmin LOWER (dVmin/dcn < 0)
  PU +10mV -> Vmin HIGHER (dVmin/dpu > 0)""")
    return "\n".join(lines)


def print_lengthscale_table(all_metrics: dict) -> str:
    """Format lengthscale comparison as text table."""
    lines = []
    lines.append("=" * 70)
    lines.append("ARD Lengthscale Comparison (smaller = more important)")
    lines.append("=" * 70)
    lines.append(f"{'Config':<25} {'mu cn':>8} {'mu pu':>8} {'mu Vop':>8} "
                 f"{'ℓ_pu/ℓ_cn':>10} {'PG>>PU?':>10}")
    lines.append("-" * 70)
    ...
    lines.append("-" * 70)
    lines.append("PG >> PU: l(common_N) < l(PU) for SNMR (PG has larger impact)")
    lines.append("=" * 70)
    return "\n".join(lines)
...
def main() -> None:
    ...
    # Extract test condition coordinates
    conditions_te = X_te[::len(VOPS), :2]  # unique (common_N, PU) pairs
    ...
    print("=" * 80)
    header = f"{'Config':<22} {'mu R²':>8} {'σ R²':>8} {'VminRMSE':>10} {'Hausdorff':>10} {'Overlap':>8} {'GradCos':>8} {'Time(s)':>8}"
    print(header)
    print("-" * 80)
    for name in CONFIGS:
        m = all_metrics[name]
        ...
        print(
            f"{CONFIGS[name]['label']:<22} "
            f"{m['mu_r2']:>8.4f} "
            f"{m['sigma_r2']:>8.4f} "
            f"{m['vmin_rmse_mV']:>8.2f}mV "
            f"{h_str:>8}mV "
            f"{m['area_overlap']:>8.4f} "
            f"{m['grad_cosine_sim']:>8.4f} "
    ...
    # ---- 7. Save gradient and lengthscale tables ----
    ...
    # ---- 8. Save metrics JSON ----
    # Remove surrogate objects from results for JSON serialization
