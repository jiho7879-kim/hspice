...
from src.toy_physics_layer import compute_vmin_from_z

# ============================================================
# Parameters (must match demo_pvta_contour.py)
# ============================================================
...
def quadrant(cn, pu):
    if cn < 0 and pu > 0: return "FSG"
    if cn > 0 and pu < 0: return "SFG"
    if cn < 0 and pu < 0: return "FFG"
    return "SSG"
...
def main():
    ...
    # Group per condition (6 Vop rows per condition)
    n_cond = len(X) // 6
    ...
    # ================================================================
    # FIGURE 1: Error Profile by Vop (2 panels)
    # ================================================================
    ...
    # annotate bias magnitude
    for v, m, s in zip(VOPS, mu_means, mu_stds):
        ax.annotate(f"{m:+.4f}", (v, m), textcoords="offset points",
                    xytext=(0, 8 if m > 0 else -14), fontsize=7, ha="center")

    # (b) Sigma error per Vop
    ax = axes1[1]
    sg_means, sg_stds = [], []
    for vop in VOPS:
        mask = X[:, 2] == vop
    ...
    ax.grid(True, alpha=0.2)
    for v, m, s in zip(VOPS, sg_means, sg_stds):
        ax.annotate(f"{m:+.4f}", (v, m), textcoords="offset points",
                    xytext=(0, 8 if m > 0 else -14), fontsize=7, ha="center")
    ...
    # ================================================================
    # FIGURE 2: Sigma error spatial maps (2x3 grid, one per Vop)
    # ================================================================
    ...
    # Build grid: for each (cn, pu), predict at all 6 Vop levels
    ...
    # ================================================================
    # FIGURE 3: Predicted vs True scatter (2 panels: mu and sigma)
    # ================================================================
    ...
    # (b) Vmin error by Vop level
    ax = fig4.add_subplot(gs[1])
    vmin_err_by_vop = {v: [] for v in VOPS}
    for i in range(n_cond):
        if not valid_vmin[i]: continue
        idx = slice(i*6, (i+1)*6)
        for j, vop in enumerate(VOPS):
            # Contribution of each Vop's error to Vmin error is non-trivial
            # Plot the per-Vop z-score error instead
            pass
    # Better: show how many conditions have Vmin valid and the Vmin distribution
    ...
    # Scatter: mu_err vs sigma_err, colored by Vmin error
    n_plot = min(1000, len(errors_mu))
    rng_idx = np.random.RandomState(42).choice(len(errors_mu), n_plot, replace=False)

    # Compute per-condition averaged errors for correlation
    ...
    ax.text(0.05, 0.95, f"r(mu_err, sigma_err)={r_mu_sigma:.3f}\nr(mu_err, Vmin_err)={r_mu_vmin:.3f}\nr(sigma_err, Vmin_err)={r_sigma_vmin:.3f}",
    ...
    # ================================================================
    # FIGURE 5: Error propagation chain (Z-score → Vmin)
    # ================================================================
    ...
    # (a) Z-score error per Vop
    ax = axes5[0]
    for j, vop in enumerate(VOPS):
        mask = X[:, 2] == vop
        ze = z_err[mask]
        ax.boxplot(ze, positions=[j], widths=0.4,
                   boxprops=dict(color=colors[j], alpha=0.6),
                   medianprops=dict(color="black", linewidth=1.5),
    ...
    # (b) Sigma error → Z error correlation
    ax = axes5[1]
    ...
    # (c) Vmin error vs sigma error
    ax = axes5[2]
    # Per-condition: mean sigma error vs Vmin error
    ...
    print()
    print("  Saved:")
    for f in ["diagnostic_error_by_vop.png", "diagnostic_sigma_error_spatial.png",
              "diagnostic_pred_vs_true_scatter.png", "diagnostic_vmin_error_decomposition.png",
              "diagnostic_error_propagation_chain.png"]:
        print(f"    results/{f}")
