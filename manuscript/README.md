...
### 소자·공정 독자용 판 D (2026-09-06) — `paper_*_D.md`

C 축약판과 **구조·수치가 같고** 두 가지가 다르다. (1) 회로 밖에서 빌려온 도구(GP, Sobol,
부트스트랩, censoring, 이분 탐색, Owen's T, 임의효과 합산)를 "원래 무엇을 하려고 만들어진
것인가 → 이 문제에 왜 맞는가" 형태로 풀어 쓰고 표 II에 한눈표를 넣었다. 통계·기계학습
배경을 전제하지 않는다. (2) 변동 축 기호를 소자 이름으로 바꿨다 — Δ는 시프트(mV), k는
무차원 배율, Δk는 pass-gate/pull-down 어긋남:

```
cn → ΔVth,N    sk → ΔVth,skew   pu → ΔVth,P
l_com → k_σN   l_sk → Δk_σN     lpu → k_σP
m_com → k_μN   m_sk → Δk_μN     mpu → k_μP
```

코드·`results/*.json`의 키는 예전 이름 그대로다. 그림(`code/gen_figures.py`)도 새 기호로
바꿔 재생성했으므로 **v4.0·B·C 본문의 표 II 기호와 그림 축 이름이 어긋난다** — 그 판들을
쓸 일이 생기면 같이 갱신할 것. 표는 C의 8개에서 9개로 늘었다(도구표 II가 들어가며 이후
번호가 하나씩 밀림).

§III-A의 전사 문장은 **한 단어**만 고쳤다 — "반올림" → "소수점을 잃었다"(실제로는 버림).
검정 전문과 부수 관측은 LEDGER **N090–N096**에만 있다. 논문에 안 넣는 이유는 그 절의 논점이
"쓰기 σ 오차는 전사 탓이 아니다" 한 줄이고 나머지는 그걸 뒷받침한 작업 기록이기 때문이다.

C 페어에 있던 한·영 누락 3건(82.7 %/625칸, +5.66 mV, 2,000 조건)도 D에서는 맞췄다 —
**C 페어 자체는 아직 QC 실패 상태다.**
