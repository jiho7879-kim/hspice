...
Verifying the minimum operating voltage (Vmin) of an SRAM across process variation is expensive by Monte Carlo (MC) and incomplete by corner analysis. We present a physics-constrained surrogate that answers both forward and inverse Vmin queries from one fixed simulation budget. A Gaussian process (GP) predicts the mean and standard deviation of the SRAM margin over nine process axes and supply voltage, while an analytic yield relation converts those statistics to Vmin. Because the conversion remains explicit and monotonic in supply, inverse queries are reduced to one-dimensional root finding rather than optimization. On production calibration data from an advanced FinFET node, the hold-out Vmin RMSE is 8.35 mV for read and 14.45 mV for write; on four PDK corners absent from training, the RMSE is 9.3 and 16.7 mV, respectively, with the limiting corner identified correctly for both modes. Axis-wise inverse queries recover threshold-voltage shifts with 2.60–3.20 mV RMSE. Global sensitivity analysis further shows that a local-mismatch axis contributes more variance to the read margin than the PMOS threshold-shift corner axis, demonstrating a limitation of corner-only variation. Finally, MC shape statistics reveal non-Gaussian read-margin behavior that can materially shift the yield-referenced Vmin; this effect is treated as a post-processing correction rather than learned by the surrogate.

**Index Terms** — Gaussian processes, SRAM, Vmin, process variation, yield analysis.
...
one-dimensional root-find solved exactly — no nonlinear optimizer or optimizer-specific stopping criterion
(Fig. 1).
...
without retraining (Section VI); and the map is numerically invertible along the validated axis
(Section IV).
...
|---|---|---|
| Gaussian process (GP) | Regression for sparse, multidimensional observations, with an estimate of prediction uncertainty | Filling in between 2,000 conditions scattered through a nine-dimensional process box, instead of re-running MC for every new condition |
...
| Owen's T | Numerical evaluation of bivariate normal probabilities | Evaluating the joint lobe term in the union probability |
...
## III. Forward Prediction Accuracy
...
### A. Error Propagation from Margin Statistics to Vmin
...
### C. Comparison With a Quadratic Response Surface
...
## IV. Inverse Vmin Estimation

The forward direction answers "what is the Vmin of this condition". The question design
asks is the reverse, and the analytic physics layer keeps the inverse query explicit. Fixing eight of the
nine coordinates leaves Vmin a one-dimensional function of the ninth; monotonicity is checked
before applying bisection — decreasing
...
±0.5 mV. **This behavior is consistent with the shared structure of the five supply-level observations.** The GP's error field is smooth, so at a given
...
maximum residual of **4.7 × 10⁻⁴ mV**, confirming numerical convergence of the surrogate solve.

### B. Vmin Boundary Estimation
...
## VI. Non-Gaussian Margin Tail and Its Impact on Vmin
...
given the per-lobe statistics it is analytically evaluable, with the joint term given by a bivariate normal CDF
...
### A. Estimation of Lobe Correlation
...
### B. Interpretation of Lobe Correlation
...
### C. Impact on the Vmin Decision

Under the adopted model, the bias enters as a shift of the threshold used in the (μ, σ) → Vmin conversion, so the correction is
the post-processing step z_bias = Z_union(ρ_LR, Z_t) − Z_t, Z_eff = Z_t + z_bias, needing no
re-simulation — one post-processing correction term. Ordering conclusions are therefore
...
## VII. Simulation-Budget Implications
...
The proposed physics-constrained surrogate estimates SRAM Vmin in both forward and inverse directions from a fixed simulation budget. On production FinFET calibration data, the hold-out Vmin RMSE is 8.35 mV for read and 14.45 mV for write, while PDK corners absent from training give 9.3 and 16.7 mV, respectively. Axis-wise inverse queries recover threshold-voltage shifts with 2.60–3.20 mV RMSE, and the Vmin boundary is obtained by one-dimensional root finding rather than a two-dimensional grid search.

The results also identify where the remaining uncertainty resides. Vmin error is more sensitive to the predicted margin spread than to its mean, and the local NMOS mismatch axis contributes more read-margin variance than the PMOS threshold-shift corner axis. Thus, corner analysis alone does not characterize the full variation window for a tail-based Vmin criterion. In addition, lobe-shape statistics indicate that the Gaussian approximation used in the conventional margin-ratio criterion can introduce a substantially larger systematic shift than the surrogate regression error. This correction is not validated against silicon and should therefore be regarded as a diagnostic result rather than a final sign-off criterion. Future work should validate the tail correction with silicon or independently measured lobe statistics and establish a shared dataset for joint read/write Vmin prediction.
