...
> Internal technical report · IEEE manuscript format · draft v5.0-**C** (2026-09-05)
> **Condensed structure (~9 pages).** `paper_en.md` (v4.0) and `paper_en_B.md` carry the
> full evidence; this version keeps only what carries the argument and reads each number
> for what it says about the cell rather than reporting it. No number contradicts the
> other versions; most are simply not repeated here.
...
>
> Figures reuse the v4.0 files: Fig. 1 = `fig1_pipeline`, Fig. 2 = `fig3_forward`,
...
group. Per-condition bootstrap standard errors enter a fixed-noise likelihood, so
conditions backed by larger MC batches carry proportionally more weight — which is why a
...
Table II lists the nine axes. Pass-gate and pull-down share the NMOS flavor and therefore
...
corr(l_PG, l_PD) ≈ 0.88 with the components sampled independently, which the variance
analysis of Section V requires. **Two of the nine axes — cn and pu — are the corner axes;
...
reused one stream across quadrants and flipped only the signs of cn and pu, giving 75% of
...
levels of a condition in the same partition. Validation is stacked at three increasing
...
training coordinates is 0/348 and 0/399). Table III collects all three.

**TABLE III. Forward accuracy at three distances from training**
...
data generated outside the training design — the model learned behavior over the (cn, pu)
...
Two consequences follow. **Accuracy is bought on σ, not on μ** — a campaign adding
conditions to improve the mean is spending in the wrong place, and Section V says which axes
carry σ. And part of the write σ error is recoverable at zero cost: **69% of the transcribed
write σ values are rounded to integers** against 1% for read. Rounding alone contributes only
1/√12 ≈ 0.29 mV of the observed 2.04 mV, and against the independent batch — 0.9% rounding,
hence a clean reference — the σ RMSE is still 1.78 mV, so the remainder is a real model
limitation concentrated at the grid floor, where σ RMSE falls from 2.041 to 1.336 mV once
0.4 V is excluded. Its signature is 33 write censoring disagreements against one for read:
near the floor the z curve is shallow, so a σ error flips the clamp decision outright.

### B. The corners, and what their spacing says about the cell

**TABLE IV. Vmin per corner — independent simulation versus surrogate (Z_t = 6.398)**

| Corner | (cn, pu) mV | Read reference → GP | Write reference → GP |
|---|---|---|---|
...
Table IV: read genuinely does not set Vmin at SFG, and that is a fact about the cell, not a
data defect.
...
in cn, since a faster pass-gate needs a lower supply, and increasing in pu. The direction is
read from the two endpoints rather than assumed. Twenty-four bisection steps shrink a
...
in the data the inverse error is measured directly in mV, and Table V gives it per axis.

**TABLE V. Coordinate recovery (245 hold-out conditions, target = reference Vmin)**
...
4.01 mV in cn and 5.11 mV in pu. The actual recovery is 60–65% of that, with bias inside
...
coordinates at nominal and extracting the iso-Vmin = 0.625 V contour over the (cn, pu)
plane gives the compliant region directly: **92.8%** of the plane passes, but the geometry
is the informative part. The boundary **exists only where pu ≥ 4.3 mV** — 33 of 70 pu rows;
if the pull-up is faster than that, any cn in ±60 mV meets spec and there is no boundary at
all. **This cell's read window closes along pu and only along pu.** Once the pull-up slows
past about 4 mV a lower bound appears on cn and rises quickly; where it is fast enough, cn
...
places the boundary within **3.7 mV of cn** or **4.7 mV of pu**. We give an in-plane
displacement rather than a Hausdorff distance because the two axes move Vmin at different
rates, so a Euclidean distance in (cn, pu) would mix non-interchangeable quantities.
...
intervals. First-order indices from the Saltelli estimator [22] appear nowhere in this
paper: their noise is the size of the index itself, most visibly where S1(l_com) = 1.020
exceeds its own S_T of 0.847, which exact arithmetic forbids and the interval
[0.706, 1.365] explains. Table VI gives the total-order indices.

**TABLE VI. Total-order Sobol indices over the training box (N = 4,096)**

| Axis | In a corner? | S_T of z, read | S_T of z, write | S_T of σ, read | S_T of σ, write |
|---|---|---|---|---|---|
| cn (NMOS Vth) | yes | **0.419** | **0.421** | 0.001 | 0.006 |
| l_com (common local σ) | **no** | **0.276** | 0.170 | **0.847** | **0.722** |
| pu (PMOS Vth) | yes | 0.188 | 0.168 | 0.007 | 0.017 |
| sk (Vth skew) | no | 0.067 | 0.097 | 0.001 | 0.011 |
| lpu (pull-up local σ) | no | 0.043 | 0.060 | 0.137 | 0.193 |
| m_com, m_sk, mpu, l_sk | no | ≤ 0.015 each | ≤ 0.032 each | ≤ 0.004 each | ≤ 0.031 each |
| **Σ** | | **1.031** | 0.987 | 1.000 | 1.007 |

**A spread axis outranks a corner axis.** For read, S_T(l_com) = 0.276 against
S_T(pu) = 0.188, with non-overlapping bootstrap intervals ([0.259, 0.293] and
...
l_com axis scales the *local* mismatch σ of the NMOS devices, and z = μ/σ depends on it
...
**98.8% (read) and 93.8% (write)** of the σ variance, l_com alone 85% and 72%, while the
...
spans a factor of 400, and by relevance l_com is indistinguishable from three other axes
...
strong, smooth, nearly linear dependence, which is exactly what l_com does to σ, earns a
long lengthscale and a large variance share at once. What survives from the fitted kernel
is direction, not ranking: λ_Vop = **4.64** is genuinely the shortest lengthscale and the
supply genuinely the most curved axis, and ARD puts λ_cn below λ_pu just as Sobol puts
S_T(cn) above S_T(pu). **Reporting ARD lengthscales as a sensitivity result is a cheap
...
computed by Owen's T [17]. The gap between the union z and the naive z of Eq. (1) is the
...
We quote the random-effects rather than the inverse-variance SE (which would give ±0.005)
...
An immediate consequence is that the experiment design of Section II-C **cannot** mitigate
...
**Table VII places this next to everything else the paper measured.**

**TABLE VII. Error sources in Vmin units, and the corner shift the correction imposes**
...
**Run together, the same cuts cost +2.6 mV** (Table VIII). The 4.3 mV gap is not noise: μ
moves the same way (2.489–2.715 mV single-factor against 3.425 mV combined), and μ and Vmin
move together.

**TABLE VIII. Combined reduction (400 conditions × reduced levels × 500 MC, same hold-out)**
...
write at 0.8 V but only −0.18 mV for read, because the GP reverts to its prior mean outside
the training range and compresses μ(V_op) in proportion to how steep that slope already is —
read's 15.1 V⁻¹ against write's 36.4 V⁻¹.
...
**Write σ is a floor, not a knob.** Section V places the σ *variance* on the local-σ length
...
(Table IV); over the window it would be a prediction on top of an unvalidated prediction, and
...
The QC audit of Section II-C in full: parse-stage typo detection (double decimal points such
