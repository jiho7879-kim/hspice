# Forward and Inverse SRAM Vmin Queries Over a Nine-Dimensional Process Window: A Gaussian Process Surrogate with an Analytic Physics Layer

**[Authors TBD — O-01]**
**[Affiliation TBD — O-01]**

> Internal technical report · IEEE manuscript format · draft v5.0-**B** (2026-09-05)
> **This is an alternative structure, not a successor.** `paper_en.md` (v4.0) reports the
> same evidence with all validation collected into one chapter. This version splits the
> paper along its two deliverables — the forward query and the inverse query — and
> subordinates everything else to what those two make possible. No number differs between
> the two versions.
...
>
> Figure files are shared with v4.0 and are cited here in the order they appear:
> Fig. 1–5 unchanged (`fig1_pipeline`, `fig2_design`, `fig3_forward`, `fig4_corner`,
> `fig5_inverse`); Fig. 6 = `fig8_sensitivity`, Fig. 7 = `fig9_skew`, Fig. 8 = `fig6_lobe`,
> Fig. 9 = `fig7_cost`.
...
Verifying the minimum operating voltage (Vmin) of an SRAM array across the process-variation
window is challenging: direct Monte Carlo is computationally prohibitive, and corner-based
sign-off cannot observe variation axes that no corner definition contains. We propose a
surrogate pipeline that answers both forward and inverse Vmin queries from one fixed
simulation budget. A Gaussian process regresses the margin statistics over nine process axes
and the supply voltage, and a physics layer with no trainable parameters converts them into
the yield-referenced Vmin. Because that layer is an analytic constraint rather than a learned
map, the inverse query — which process condition meets a target Vmin — is solved exactly, one
axis at a time, so a Vmin boundary is identified directly instead of by grid search: tracing a
planar boundary takes 858 surrogate evaluations against 4,900 for a grid over the same plane,
and returns the boundary points themselves rather than interpolants. On production
calibration data from an advanced FinFET node the margin mean is predicted with R² = 0.9965
for read and 0.9989 for write, and the condition-level hold-out Vmin RMSE is 8.35 mV for read
and 14.45 mV for write; on PDK corners absent from training it is 9.3 and 16.7 mV, with the
limiting corner of each mode identified correctly, and on an independently designed batch
evaluated without retraining it is 4.26 mV. Inverse queries recover a process coordinate from
a Vmin value to within 2.6–3.2 mV, finer than the forward error implies. A variance
decomposition of the same surrogate reveals that the NMOS common local-σ axis carries more of
the read margin variance than the PMOS threshold shift does, and that at least 39% of that
variance sits on axes no corner contains. Additionally, we quantify a systematic bias in the
standard z-score method — comparable to the entire margin budget — and introduce a low-cost
diagnostic that measures it from production Monte Carlo shape statistics alone: the lobe
correlation ρ_LR = −0.371 corresponds to 70 mV of Vmin.
...
### A. Verifying Vmin across the process window
...
The industry standard estimates Vmin by Monte Carlo (MC). For each process condition,
thousands of random mismatch samples build a margin distribution, repeated at every voltage
level. At the scale used here — 2,000 conditions per mode × 4–5 voltages × 5,000 MC per
condition — that is 4–5 × 10⁷ circuit simulations. With a simulator run of minutes to tens of
minutes, the wall-clock time is weeks to months even under parallel execution, plus PDK
licences, concurrency limits, and server cost. The burden grows at advanced nodes as compact
models get heavier.

Restricting verification to the representative corners (FFG/FSG/SFG/SSG) contains the cost
but loses two things.

First, a corner is only an extreme combination of two axes, the NMOS and PMOS Vth shifts.
The Vth skew, local σ, and mobility axes studied here do not enter the corner definition at
all. Section VI quantifies their contribution; a corner-based procedure cannot observe those
axes even in principle.

Second, corners answer only the forward question. The questions asked in practice are
inverse ones — which combination of variations violates the spec, how far must a parameter
be tightened to restore compliance, how much skew can be tolerated. A finite set of points
fixes neither the location nor the shape of the compliance boundary.

### B. What a replacement has to do

A surrogate that is to stand in for either method must therefore satisfy three requirements
at once, and it is the third that shapes the design of this pipeline.

1. **Accuracy at the decision.** The predicted Vmin must be close enough to what MC would
   have produced that a sign-off screen can act on it, over the whole nine-dimensional
   window and not only near nominal.
2. **Coverage.** All nine variation axes must be inputs, so that the axes no corner contains
   are observable rather than assumed away.
3. **Invertibility.** The same fitted object must answer "which process condition meets a
   target Vmin", because that is the question design actually asks.

Requirement 3 is the one that constrains the architecture. If the map from process
coordinates to Vmin were learned end to end, inverting it would mean optimizing through a
black box. We instead split the map: the Gaussian process learns only the margin statistics
(μ, σ), and the conversion from those to Vmin is imposed as an **analytic constraint with no
trainable parameters**. Because the constraint is analytic and monotone in the supply voltage,
the inverse along any single axis reduces to a one-dimensional root-find that is solved
exactly — no optimizer, no learning rate, no convergence tolerance. Fig. 1 shows the
resulting pipeline, and Section III-F gives the solution.
...
1. **A surrogate pipeline that serves forward and inverse Vmin queries from one fixed
   budget.** The physics layer is an analytic constraint with no trainable parameters, which
   is what makes the inverse tractable (Section III).
2. **Forward accuracy validated at three increasing distances from the training
   distribution** — in-batch hold-out, PDK corners absent from training, and an
   independently designed pilot batch — for both read and write modes (Section IV).
3. **An inverse query solved as an axis-wise one-dimensional exact solution**, accurate to
   machine precision, which identifies a Vmin boundary directly and 5.7× more cheaply than
   a grid search over the same plane while returning exact boundary points (Section V).
4. **A quantitative sensitivity ranking over the full window**, which finds the NMOS common
   local-σ axis outranking the PMOS threshold shift for read, and at least 39% of the margin
   variance on axes no corner definition contains (Section VI).
5. **Identification of ρ_LR as the physical quantity that sets the systematic optimism of
   the min-statistics z-score**, plus a low-cost diagnostic that measures it from production
   MC shape statistics alone and converts it into the Vmin units of a product spec
   (Sections II-D, VII).
6. **Evidence for budget reduction** along voltage levels, condition count, and MC depth,
   including the direct measurement that the losses do not multiply when the three are cut
   together (Section VIII).
...
Equations (1)–(3) are the whole of the physics layer. Nothing in them is fitted, which is
why the same expression can be run in either direction.
...
the minimal bracket around the spec point. Section VIII-A gives the quantitative
...
corrected threshold, not the naive one (Section VIII-A).
...
crossing must reach, and the correction requires no re-simulation — it edits one constant in
the physics layer. Consequently the **ordering** conclusions of Sections IV–VI — the
sensitivity ranking and the corner ordering — are unaffected. Anything referenced to the
threshold does move: pass fractions, tolerance widths and the location of the compliance
boundary all contain Z_t, so both thresholds are reported wherever they occur
(Tables XIII and XVI).
...
more efficient than a tail fit that depends on a handful of lower quantiles. This is what
makes the diagnostic of Section VII low-cost: the shape statistics of an MC run the fab has
already paid for are sufficient, and no new deck is needed.

---

## III. The Surrogate Pipeline
...
multipliers decompose the same way. Two of the nine — cn and pu — are the corner axes; the
other seven are outside any corner definition, which is what makes the sensitivity result of
Section VI possible.
...
retention-side minimum) at 125 °C, 2,000 conditions × 5 levels, and V_trip at −40 °C,
2,000 conditions × 4 levels. The intersection of their nine-dimensional coordinates is
**0/2000**, and the input has no temperature axis, so **the two cannot be merged into one
GP.** There are two models; only the argument of the paper runs jointly.

Both the price and the benefit of this choice surface in Section IV-B. The price is that the
per-condition combined Vmin = max(read, write) cannot be verified against the reference
simulation in the 2,000-condition batches. The benefit is that the **complementarity** of the
two modes — each filling the other's censored corner — is confirmed by the reference
simulation.

### B. Design of the condition set

**Common/skew parameterization.** The pass-gate and pull-down devices share the NMOS
flavor, hence share the dominant variation sources — gate stack, channel doping, anneal,
lithographic critical dimension — with imperfect tracking from device geometry and layout
environment. Sampling them independently assigns design points to states never realized in
silicon, where two devices of the same flavor diverge in opposite directions at the
mismatch level.
...
variance-based analysis of Section VI requires. Fig. 2 shows both halves of the design:
...
**Quadrant weighting.** Read and write margins degrade in different quadrants — FSG for
the former, SFG for the latter. A separate deck set per metric carries the weights of
Table III, raising the resolution of the worst region by 2–4× at a fixed condition count.
Conditions are generated from deterministic PCG64 draws on independent per-quadrant
streams.
...
**Mirror-twin leakage — an early design failure and its fix.** An early pilot design
reused a single quasi-random stream across the four quadrants, flipping only the signs of
cn and pu. As a result, 75% of conditions had a mirror twin sharing the remaining seven
coordinates, and in a random hold-out about 74% of test conditions had a twin in training
— **inflating the accuracy metrics with no implementation defect anywhere.**
...
from 14.74 to **8.35 mV**. Section IV-A describes what triggered the audit — **the model
...
**The repeatability floor.** Both sheets of the independent pilot batch of Section IV-C
accidentally contain a duplicated condition — the same coordinates run twice. The difference
between the two repeats is max |Δμ| = 0.34 mV and ΔVmin = 1.09 mV for read, and 0.70 /
0.64 mV for write. That **1.1 mV** is the resolution of the reference itself, and no error
figure in this paper is meaningful below it. It is a per-condition repeatability, not a bound
on the sampling error of an RMSE over hundreds of conditions, and it is used that way
throughout.
...
accepts per-condition noise directly. Appendix A gives the background, and Section IV-D
reports what the choice actually buys.
...
**Input standardization.** The input vector mixes mV-scale shifts, V-scale supply levels,
and dimensionless multipliers. Without standardization, marginal likelihood optimization
converges to a markedly worse optimum **with no diagnostic warning**. Most of an improvement
initially attributed to the physics constraints was later traced to this factor. All inputs
are standardized with training statistics.

**Physics constraints.** Three constraints inject prior device knowledge. **Corner
anchoring** augments the training set with virtual observations at the four global corners,
preventing extrapolation drift at the domain extremes. A **monotonicity penalty**
ReLU(−∂μ/∂V_op)² evaluated at probe points through the posterior suppresses the unphysical
prediction that raising the supply degrades mean stability. Weak regularization induces a
linear σ(V_op) trend consistent with established mismatch scaling [19].

**Noise-aware likelihood.** Per-condition bootstrap standard errors enter a fixed-noise
Gaussian likelihood, so conditions backed by larger MC batches receive proportionally larger
weight. Bootstrap rather than analytic standard errors are used because the standard error of
σ is sensitive to kurtosis.
...
rather than depth per condition (Section VIII-C).

### E. The physics layer, and the forward query

The conversion from (μ, σ) to Vmin is imposed as an **analytic constraint with no trainable
parameters**: Eqs. (1)–(3) exactly as written. For each condition the posterior mean is
evaluated at every supply level, the margin ratio is formed by (1), and the crossing with
Z_t is interpolated linearly. Censored conditions are flagged and excluded.

That this layer is not learned buys three things, and they are the reason the architecture is
split this way rather than fitted end to end.

- **Extrapolation stays physical.** In sparsely sampled regions the GP extrapolates only
  (μ, σ), while the definition of Vmin never moves.
- **The threshold is editable.** The correction of Section II-D.2 changes one constant and
  requires no re-simulation and no retraining.
- **The map is invertible in closed form along any single axis**, which is Section III-F.

The forward query is then a single posterior evaluation per supply level followed by an
interpolation — milliseconds per condition, against a MC batch at every level.
...
rather than clipped to a range end. That distinction is part of the result (Section V-C).
...
each row of the pu grid, which replaces the grid search that would otherwise be needed.
Section V-D reports the cost.
...
This chapter answers one question at three increasing distances from the training
distribution — **within how many mV does the surrogate reproduce the worst-case Vmin that
HSPICE MC would have produced at an arbitrary process condition?** The criterion is the
per-condition Vmin; the accuracy of μ and σ is reported alongside as the intermediate product
that makes it.

The split is at condition level. Of each batch's 2,000 conditions, 1,700 go to training and
300 to hold-out, and all supply levels of a condition always stay in the same partition. As
established in Section III-B neither batch has mirror twins, so the condition-level split
leaks nothing, and the training script asserts this. Vmin errors are reported on the
non-censored subset with the censoring share alongside.
...
magnitude below the metric bias measured in Section VII (+1.054 σ in z, 70 mV in Vmin).
Fig. 3 plots predicted against reference Vmin for both modes.

**Fig. 3.** Reference-simulation versus predicted Vmin on the hold-out conditions.

**The write bottleneck is the variance, not the mean.** In Table IV the write μ prediction
is more accurate than the read one (R² 0.9989). Its Vmin error is nevertheless 1.7× larger,
and the reason is σ. Since z = μ/σ, the relative error of σ propagates straight into z and
then into the crossing. The write σ reaches only R² = 0.732.
...
1/√12 ≈ 0.29 mV, which does not account for the observed 2.04 mV. Section IV-C tests this
...
**A by-product: the surrogate as a transcription-error detector.** In early training two
hold-out conditions had Vmin errors above 100 mV. Tracing them led not to the model but to
the transcription sheet. In both, only the 0.6 V SNM mean was recorded at one tenth of its
neighbors (e.g. 10.9 mV between 83.8 and 127.0 mV), and the surrogate was predicting
109.9 mV for the same cell. This triggered the full audit of Section III-C. Using **a trained
surrogate to point at defects in the data that trained it** is a secondary but practical
by-product in any internal flow with heavy transcription.

### B. PDK corners absent from training

The hold-out of Section IV-A comes from the same batch, so a bias in the design itself would
...
A model can fit the numbers and still be wrong about the device, so Table VI lists the
...
evidence ledger. These are qualitative checks; the quantitative sensitivity ranking is read
from the variance-based indices of Section VI-A, and Section VI-B explains why the two must
not be confused.
...
**The two modes fill each other's gaps.** In Table VII, SFG is clamped below 0.4 V in both
reference and prediction for read. This is a physical fact, not a data defect — **read does
not set Vmin at that corner.** SFG combines a fast pull-up with a slow pass-gate, which
favors read stability while making the cell hard to flip, so **write** becomes the
constraint. Symmetrically, FSG is censored for write.
...
coordinates, and that would be a prediction built on a prediction (Section IX-C). Fig. 4
compares the per-corner values for both modes.

**Fig. 4.** Reference-simulation versus predicted Vmin per corner, read and write.

### C. An independently designed batch

The hold-out came from the **same batch** as training — same generation, same deck template,
same MC settings. The stronger question is what survives a move to an **independently
designed batch**.

**The external batch.** The Stage-B pilot batch was designed earlier and independently of
the nine-dimensional batches, and contains both metrics: 348 read conditions and 399 write
conditions. It sweeps only (cn, sk, pu) and fixes the six length/multiplier axes exactly at
nominal, so every condition lies on a three-dimensional subspace of the nine-dimensional
space. The script asserts three things: **inside the training box** (not extrapolation),
**no leakage** (coincidence with the 1,700 training coordinates is 0/348 for read and
0/399 for write), and **no retraining** (the Section IV-A checkpoints are evaluated as they
are).
...
**Self-consistency audit of the pilot batch.** Stage-B is a **working file, not final raw
data**. Compared with its backup, nine cells have already been corrected by hand — six
decimal-point errors, and three values of one condition **shifted by one row**. We audit on
the assumption that defects remain.
...
write (robust SD 1.00 / 0.93 mV; worst mean residual −54.4 / −35.4 mV). Two independent
confirmations follow. First, the flagged deviations are tens to hundreds of times the
1.09 mV repeatability floor established in Section III-C from the duplicated conditions in
these same sheets. Second, the 13 read conditions flagged this way contain **all five** of
the largest Vmin errors: two independent criteria point at the same conditions. No
correction was applied — that requires the original decks (O-06).

**Results.** Table VIII gives the read batch and Table IX the write batch, each with and
without the conditions the batch's own consistency audit flags.

**TABLE VIII. External validation — read (all five levels inside the training range)**
...
**TABLE IX. External validation — write (scored on the trained range 0.4–0.7 V)**
...
**The two modes say different things.** For read, with defects excluded, the external
batch beats the in-batch hold-out (4.26 vs 8.35 mV). This is not only an absence of
overfitting; **the plane is easier**. The pilot plane fixes the six length/multiplier axes
at nominal, and those axes are precisely what creates the condition-to-condition spread of
σ (the pilot's σ is nearly constant at 13.4 ± 0.27 mV across all conditions). With the
denominator of z = μ/σ effectively fixed, σ RMSE falls from 0.256 to 0.146 mV, z RMSE from
0.373 to 0.137, and the Vmin error follows. The representative figure for the full
nine-dimensional window remains the **8.35 mV** of Section IV-A. **This is also an
independent prediction that the variance decomposition of Section VI-A will name the
length/multiplier axes as the main contributors to σ — the two sections cross-check each
other.**

For write, external and in-batch are nearly identical (13.63 vs 14.45 mV). The plane does
not get easier, because the write bottleneck is σ and the write σ does not shrink when
the length/multiplier axes are fixed. This yields **an independent answer to the σ
transcription question raised in Section IV-A**. The nine-dimensional write batch has 69.3%
of its σ values rounded to integers; the pilot has **0.9%** — effectively no rounding, hence
a much cleaner reference. Against that clean reference the σ RMSE is still 1.78 mV, so
**rounding is part of the cause of the write σ error but not all of it.** The remainder is a
genuine limitation of the model. The 33 write censoring disagreements (against one for read)
are the most direct evidence of the same root cause: near the 0.4 V floor the z curve is not
steep, so a σ error flips the clamp decision outright.

**Voltage extrapolation — write at 0.8 V.** The nine-dimensional write batch has 0/2000
entries at 0.8 V, so the model was trained only over 0.4–0.7 V. The pilot batch has 400
rows at 0.8 V, giving a direct check of extrapolation **one level outside the training
range**. μ RMSE **7.55 mV**, μ bias **−6.45 mV**, R² 0.969, σ RMSE 1.19 mV, z RMSE 0.854.
Most of the error is **systematic bias**: the model consistently underestimates the write
margin at 0.8 V. Against the interpolation-range μ RMSE of 2.08 mV this is 3.6×.
Voltage-axis extrapolation produces bias even one level out, and that fact constrains the
grid reduction argument of Section VIII-A.

### D. How much of this is the Gaussian process?
...
layer, on the same hold-out. Table X puts the two side by side.

**TABLE X. Gaussian process against a quadratic response surface**
...
treatment, the axis-wise inverse, the sensitivity indices, the budget curves and the ρ_LR
correction all take μ and σ as inputs and would accept either model. What this comparison
says is that over a process box this smooth, a quadratic surface is sufficient for the mean
and the spread — which is useful news for a fab flow, since it removes GP training from the
deployment path entirely, and it means the contribution of this paper is the *pipeline*, not
the regressor inside it.
...
condition in this dataset carries the same 5,000 samples (Section VIII-C). We therefore do
...
knee of Section VIII-B is a property of the GP — a 66-coefficient surface would be expected
to need far fewer conditions, which we did not measure.

### E. Summary

Three levels of validation return the same order of magnitude for read — 8.35 mV in-batch,
9.3 mV on independent corners, 4.26 mV on an independent batch — and the write mode is
consistently worse by a factor of roughly 1.7, for a reason now localized to σ. That is the
accuracy on which the next two chapters rest.

---

## V. Inverse Queries and the Design Boundary

The forward surrogate answers "what is the Vmin of this condition". The use this paper
targets is the reverse — **"which process condition meets a target Vmin?"** This chapter
validates that direction against the reference simulation, extracts the compliance boundary
it produces, and reports what it costs.

### A. Coordinate recovery
...
rather than clipped. Table XI gives the recovery error for each axis.

**TABLE XI. Coordinate recovery error (245 hold-out conditions, target = reference Vmin)**
...
solution is unique, and this residual confirms that the axis-wise solver of Section III-F is
exact to machine precision — the property that distinguishes it from an optimizer, which
would have to be stopped at a tolerance.

### C. The design boundary
...
read figure by the measured |∂Vmin/∂x| of Table XI turns it into an in-plane displacement:
...
Read the result as follows. **The read process window of this cell closes only along pu.**
...
Because the boundary is solved rather than searched, its cost is set by the number of rows,
not by the resolution of a grid. Solving the 33 pu rows where a boundary exists took 26
evaluations each, **858 condition evaluations** in total, against **4,900** for a 70 × 70
grid over the same plane — a 5.7× difference. The two are not equivalent products either:
where the grid can only place boundary points by interpolating between cells, the axis-wise
solution returns the boundary points themselves, so the 5.7× understates the gap.

Without the surrogate the query does not exist at all. One bisection requires Vmin at 26
...
## VI. Sensitivity Across the Full Window

The surrogate covers all nine axes, so it can answer a question the corner flow cannot pose:
which axes actually move the sign-off margin. This chapter asks that, then checks the free
proxy for the same question — the fitted ARD lengthscales — and finds it misleading, then
converts the ranking into the tolerance a designer asks for.

Nothing here refits. Both trained models are loaded from the checkpoints of Section IV-A,
and the decomposition costs 45,056 surrogate evaluations and no new simulation.
...
time-zero spec voltage of Table I, z(V_T0) is finite everywhere, is monotone in the sign-off
margin, and is the quantity the T0 decision actually reads. μ(V_T0) and σ(V_T0) are
decomposed as well, because σ is the write bottleneck of Section IV-A and the subject of an
open prediction from Section IV-C.
...
**TABLE XII. Total-order Sobol indices over the training box (N = 4,096; 95% bootstrap
...
**1) The NMOS common local-σ axis outranks the PMOS threshold shift.** For read,
S_T(l_com) = 0.276 against S_T(pu) = 0.188, and the bootstrap intervals do not overlap. A
corner definition contains pu and does not contain l_com. The axis that a corner-based
procedure would consider the second most important knob is in fact the third, and the axis
above it is one the procedure cannot set.

**2) Two fifths of the margin variance is invisible to corners.** A corner moves only the
...
bound is close to tight. This is the quantitative form of the limitation stated in
Section I-A: the gap is not a rounding effect, it is two fifths of the problem.

**3) σ is a length story, and only a length story.** The three local-σ length axes carry
**98.8% (read) and 93.8% (write)** of the σ variance, and l_com alone carries 85% and
72%. The three multiplier axes together carry 0.3% and 2.9%. Section IV-C predicted, from
...
**The first-order indices are too noisy to interpret, and we do not.** S_T is the
better-determined of the two: no total-order interval exceeds 0.05 in the z columns or
0.07 in the σ columns, in either mode. S1 is not. The read z column gives S1(cn) = 0.302
with an interval of [0.199, 0.416], and the nine S1 estimates sum to 0.790 while the nine
S_T sum to 1.031. For a purely additive response both sums would be 1. The S_T excess of
0.031 says interactions are small; the S1 deficit of 0.21 is within the combined noise of
nine estimates each carrying a ±0.1 interval. The two statements cannot both be sharp, so
we rank on S_T and quote no interaction fraction.
...
**Fig. 6.** Total-order Sobol indices with bootstrap intervals for (a) z(V_T0) and
(b) σ(V_T0), read and write; (c) the ARD relevance of the same axes, in the same order.

### B. A fitted lengthscale is not a sensitivity

ARD gives one lengthscale per input, learned from the data, so it is tempting to read the
fitted values as a free sensitivity ranking. Since inputs are standardized over the training
box, the values are comparable across axes, and relevance can be defined as the normalized
inverse lengthscale λ⁻¹/Σλ⁻¹. For the read μ kernel the nine device lengthscales span 7.41
(cn, the shortest) to 8.41 (l_sk), a relevance spread of 0.108–0.123 against the 0.111 an
axis would get if all nine were equal. The write model behaves the same way (device axes
6.71–8.82, λ_Vop = 4.00). The σ kernel is flatter still: **all nine device lengthscales fall
inside 7.83–7.92**, a spread of about 1%. Read literally, the fit says no process axis
matters more than any other for σ.

Panels (a) and (c) of Fig. 6 rank the same nine axes with the same model, and they disagree.
ARD relevance spans a factor of 1.13 across the nine axes; S_T spans a factor of 400. The
order disagrees too: by ARD relevance l_com is indistinguishable from three other axes (all
0.109 at the precision of Table XII), yet it ranks **second of nine** by variance share. The
σ kernel is the extreme case — nine lengthscales within about 1% of each other, while one of
those axes carries 85% of the σ variance.
...
What survives is narrower than a ranking. λ_Vop = 4.64 is genuinely the shortest
lengthscale in the model, and the supply axis is genuinely the most curved one. The
direction of the pass-gate check in Section IV-B also holds up: ARD puts λ_cn below λ_pu,
and Sobol puts S_T(cn) above S_T(pu). Directions can be read off a fitted kernel; rankings
and magnitudes cannot. This is the caution referred to in Appendix A, and it applies to any
GP surrogate whose lengthscales are reported as a sensitivity result.

### C. Skew tolerance

Section VI-A measured how much the threshold skew sk contributes on average. A designer
asks the sharper question: with the rest of the process fixed, how far can sk drift before
the T0 margin is gone? sk is a non-corner axis, so this question has no answer inside the
corner flow at all.
...
correction of Section VII raises the bar the margin must clear. Table XIII summarizes the
result and Fig. 7 maps it.

**TABLE XIII. Skew tolerance over the (cn, pu) plane (625 cells, sk swept ±20 mV).**
**The write Z_eff columns apply the read-measured z_bias, which is an assumption: write
ρ_LR has not been measured (Section VII-E).**
...
z_bias transfers between modes; since write ρ_LR is unmeasured, that column is a
projection, not a measurement.

**Where it closes is where the paper has been pointing all along.** Fig. 7 shows the
tolerance collapsing in the fast-NMOS / slow-PMOS region — the FSG quadrant that
Section IV-B identifies as the read-limiting corner and Section VII-D pushes past spec. The
write mode keeps more cells (97.0% retain some margin at Z_eff) but loses more inside them:
its IQR falls to 30.6–40.0 mV, consistent with the larger sk variance share in Table XII
(0.097 versus 0.067).

**Fig. 7.** Passing skew width over the (cn, pu) plane, read mode, (a) at Z_t = 6.398 and
...
axes at nominal, and Table XII says l_com moves the read margin more than sk does, so a
joint tolerance region over several axes would be tighter than this one. And the map is a
surrogate prediction: cells within roughly one model error (8.35 mV in Vmin,
Section IV-A) of the boundary should be read as boundary cells, not as decided ones.

---

## VII. The Min-Statistics Bias and a Low-Cost Diagnostic

The problem raised in Section II-D is closed here by measuring ρ_LR. The point of this
chapter is not only the number but the **protocol**: the measurement uses shape statistics of
Monte Carlo output the fab already produces, so any flow can obtain it without a new deck.

### A. Rejecting the Gaussian hypothesis
...
kurtosis, and the observed minimum. Three statistics reject normality independently, and
Table XIV collects them.

**TABLE XIV. Three independent pieces of evidence against normality (9 conditions)**
...
### B. Estimating ρ_LR — two paths that converge
...
implementation against the fab's implementation of the same model, not the model itself.
The standard error of g₁ is taken not as the Gaussian null √(6/n) = 0.00775 but as
**0.00816**, simulated directly from the min-of-two distribution (400 repetitions). By the
delta method, SE(ρ̂) = 0.013–0.016.
...
conditions estimate a common ρ, and the uniformity test rejects exactly that presumption
(χ² = 53.6 on 8 degrees of freedom, p = 8.1 × 10⁻⁹). The wider interval is also the one
consistent with the two estimation paths themselves, which differ by 0.027. In Vmin terms
±0.013 in ρ is about ±1 mV, so the conclusions below are unchanged by the substitution.
...
**The rejected uniformity test does not force a per-condition correction.** ρ_LR does vary
significantly across conditions, but the **effective size of that variation is
negligible**: converting each condition's ρ̂ into a z_bias spans +0.981 … +1.095 σ, which
in the spec band is ±4 mV of Vmin — less than half the forward model error of 8.35 mV. **A
single scalar z_bias is therefore sufficient**, and the justification is not "the
uniformity test passed"; it is **"the variation is smaller than the model error"**. At
n = 10⁵ even practically meaningless differences become significant, and this is such a
case.

### C. Conversion to Vmin
...
36.4 V⁻¹ for write (IQR 31.9–42.3); the local slope at the read-limiting corner FSG is
14.2 V⁻¹.
...
less than the linear conversion because of curvature). Table XV applies that shift to the
four corners.

**TABLE XV. Read Vmin per corner before and after correction (125 °C)**
...
Fig. 8 shows the two estimates side by side and the shift the correction imposes on the
corners.

**Fig. 8.** (a) ρ_LR from the two estimation paths; (b) the shift the correction imposes
on the corner Vmin values.

### D. After correction, the read-limiting corner exceeds spec
...
between-condition uniformity is rejected (Section VII-B), so applying the pooled z_bias at
FSG is an **extrapolation into an unsampled quadrant**; a corner-labeled re-measurement
is what would close it, and it is item 3 of Section IX-G. And 0.662 V is the worst value
in **corner space**, where the other seven axes are held at nominal — Section VI-A puts at
...
1. **Corner sign-off passed only thanks to min-statistics optimism.** The naive FSG value
   ...
   tail shape, and the bias measured here is twice it.
...
3. **The earlier "silicon upper-bound consistency" argument is withdrawn.** It used
   ...
   silicon Vmin measurement, which this study does not have (Section IX-A).

### E. The diagnostic protocol, and what it does not yet cover

The measurement above cost no new simulation: nine conditions that were being run anyway,
with four extra columns exported. That is the protocol we recommend adopting as standard —
record the skewness and the lower quantiles of the minimum, and preferably the per-lobe
statistics (μ_L, σ_L, μ_R, σ_R, ρ_LR) themselves. Three gaps remain in what we could measure
from the present export.
...
- **Direct per-lobe statistics.** ρ_LR is inverted from the distribution shape and
  therefore presumes the min-of-two model. The two estimation paths of Section VII-B share
  that premise, so their agreement tests the estimator and not the premise. Recording the
  per-lobe statistics directly for a few conditions would turn the premise into a
  verification.

---

## VIII. Simulation Budget

The pipeline above was built from a campaign whose cost is a product of three factors.
...
**the same 300-condition hold-out as Section IV-A**, so the curves are mutually comparable,
and no experiment touches the hold-out — only the training budget is reduced.

### A. Voltage levels

**The spec decision cannot see the top level, by construction.** Since the spec voltage
0.625 V lies inside [0.6, 0.7] V, z(0.625 V) is a linear interpolation of those two points.
The 0.8 V sample **cannot participate** in the decision — this is the structure of the
interpolation formula, not an empirical approximation. Comparing z(0.625 V) from the full
five-level grid against the same value from the 0.6/0.7 pair alone over all 2,000
conditions gives max |Δz| = **1.78 × 10⁻¹⁵**, machine epsilon. **If the decision is the
only deliverable, two levels suffice and 60% can be removed.** That is the upper bound; the
rest of this section asks whether it can actually be taken.

**But Vmin contours do use the top level — and the correction increases that use.** Vmin
is the crossing z(V) = Z_t, so each condition needs a **bracket** around its own crossing.
We examine both before (Z_t = 6.398) and after (Z_eff = 7.453) correction, and Table XVI
gives the resulting distribution.

**TABLE XVI. Distribution of crossing brackets.** The write Z_eff column applies the
read-measured z_bias; write ρ_LR is unmeasured (Section VII-E).
...
**Which level can be removed differs by mode.** The 0.6/0.7 bracket supports the spec
decision and cannot be removed in either mode (the script asserts this). That leaves
exactly one candidate per mode: **0.8 V** (top) for read and **0.4 V** (bottom) for write,
since the write grid already ends at 0.7 V.
...
dz/dV_op of 15 V⁻¹ is gentler than the write 36 V⁻¹, so the absolute compression is
smaller. **The decision to trim a grid end is conditional on that mode's slope.**
...
cubic — the fit times show it. Table XVII gives the result.

**TABLE XVII. Effect of the training condition count (read)**
...
floor of Section III-C — though that floor is a per-condition repeatability and not a bound
on the sampling error of an RMSE over 243 conditions, so it is an indication, not a proof.
...
n′ to the noise-aware GP. The hold-out labels are untouched. Table XVIII gives the result.

**TABLE XVIII. Effect of MC depth (read)**
...
fit-to-fit noise band identified in Section VIII-B. **Cutting the MC depth tenfold, from
5,000 to 500 samples per condition, produces no measurable loss.**
...
bites harder than it did at 1,700 conditions.** The flatness of Section VIII-C is a result
obtained with 1,700 neighbors. So we ran the combined experiment directly, and Table XIX
reports it.

**TABLE XIX. Combined reduction (400 conditions × reduced levels × 500 MC, same hold-out)**
...
−1.7 mV, and each one on its own lies inside the ±1 mV fit-noise band of Section VIII-B. The
...
Fig. 9 collects the three single-factor curves and the combined point.

**Fig. 9.** Budget-reduction Pareto — (a) conditions, (b) MC depth, (c) single factor
versus all three.
...
reduction.** The ρ_LR measurement of Section VII-B required **10⁵** samples per condition
...
### A. Where the uncertainty actually sits

Table XX places this paper's numbers side by side, and the priority is clear.

**TABLE XX. Error sources, in Vmin units**
...
three times all of them combined. Effort spent making the model more accurate ranks below
it, which is also why the quadratic/GP gap of Section IV-D changes nothing that matters.
...
(Section VII-E). The **correction is post-processing**, Eqs. (6)–(7), requiring no
re-simulation. But it is **not verified against silicon.** The earlier claim that inverted
an upper bound from corner simulation and reported the measurement as "inside the bound"
was circular and has been withdrawn: corner simulation is the very source of the naive z
being corrected, so it cannot serve as independent evidence for the size of the correction.
An independent check requires an actual silicon Vmin measurement, which this study does not
have.

### B. The limit of write σ prediction — the diagnosis is only half confirmed

Section IV-A attributed the write σ error (RMSE 2.04 mV, R² 0.732) to a wide
condition-to-condition spread plus the 69% integer rounding of transcribed σ values.
Section IV-C tested that diagnosis **independently**: the pilot batch, with only 0.9%
...
disagreements (one for read), and Section VIII-A localized much of the write σ error to the
...
Section VI-A supplies the axis-level detail that was missing from both: the σ **variance**
...
and the two are not the same. Section IV-C is the direct test: the pilot batch freezes all
...
lie 2 mV apart is a reference-simulation result. **What is not possible**: a combined
contour over the full 2,000-condition window. Each surrogate could be evaluated at the
other's coordinates to *propose* a combined decision, but that is a prediction built on an
unvalidated prediction, and this paper does not report that surface as a result.
...
Section IV-D scored a 66-term quadratic response surface against the GP and found it at
...
difficulty**; the difficulty is the metric (Section IX-A), the censoring, and the labels.
Three consequences follow.

First, the paper's results are portable: every downstream step consumes only μ and σ, so
a flow that prefers least squares to a GP keeps the physics layer, the inverse, the
sensitivity indices, the budget curves and the ρ_LR correction unchanged. The contribution
is the pipeline, not the regressor.
...
same 5,000 samples. A campaign with genuinely heterogeneous depth, which Section VIII-E
argues for on other grounds, is also what would make that likelihood earn its place.

Third, the budget conclusions of Section VIII are GP-specific. A 66-coefficient surface
...
The voltage-level reduction of Section VIII-A is conditional on the spec of Table I; if the
...
1. **The spec sets the voltage levels.** The bracket around the spec point cannot be
   ...
   (Section VIII-A); in a steep mode, one level of extrapolation produces 5–6 mV of
   systematic bias.
2. **Spend the budget on breadth of condition coverage.** Cutting MC depth tenfold
   ...
   the training cost (cubic). Where breadth is added, Section VI-A says where it pays: the
...
3. **Re-measure the price when cutting all three factors together.** The product of
   single-factor curves is optimistic relative to the combined run (Section VIII-D).
...
calibration data from an advanced FinFET node. Its defining feature is that the conversion
from margin statistics to Vmin is an analytic constraint with no trainable parameters, which
is what makes the same fitted object usable in both directions.

**Forward.** On a condition-level hold-out the margin mean is predicted with R² = 0.9965
for read and 0.9989 for write, and the Vmin RMSE is 8.35 mV for read and 14.45 mV for
write. The error does not grow on the three scorable of four PDK corners absent from
training (9.3 / 16.7 mV), and the limiting corner of each mode is identified correctly.
Applied without retraining to an independently designed pilot batch, the read Vmin RMSE is
21.4 mV over all 348 conditions and 4.26 mV over the 283 that pass that batch's own
consistency audit. Three levels of validation return the same order of magnitude.
...
4.7 × 10⁻⁴ mV), and it identifies a Vmin boundary directly rather than by grid search: 858
evaluations against 4,900 for a grid over the same plane, returning exact boundary points
rather than interpolants, and located to within 3.7 mV of cn. Its output shows a structure
corner sign-off cannot see: the read process window of this cell closes only along pu, and a
lower bound on cn appears once PU slows by more than 4.3 mV.

**Sensitivity.** A variance decomposition of the same surrogate, taken under a uniform
prior over the design ranges of Table II, finds the NMOS common local-σ axis carrying more
of the read margin variance than the PMOS threshold shift does, and puts at least 39% of the
read margin variance and 41% of the write on axes that no corner definition contains. The
same decomposition resolves the σ variance — 98.8% of it sits on the three local-σ length
axes and 0.3% on the multipliers — which sharpens an observation Section IV-C could only
make about the six axes as a block. The fitted ARD lengthscales, the free proxy for the same
...
**The metric.** The systematic optimism of the min-statistics z-score is set by the lobe
correlation ρ_LR, which can be measured from the shape statistics of production MC output
alone — a diagnostic that costs no new simulation. The pooled value on which two
independent paths converge, ρ_LR = −0.371 (random-effects SE 0.013), corresponds to
+1.054 σ in z and 70 mV in Vmin, an order of magnitude above the surrogate regression
error and comparable to the entire margin budget. Correcting for it puts the read-limiting
corner FSG 37 mV past spec.
...
deltas sum to −1.7 mV and predict no degradation at all. Reductions must be reported as
Pareto points with a stated price, not as lossless.
...
sensitivity measure. Section VI-B examines that reading critically.
...
residual criterion was used instead (Section IV-C). The criteria differ because the pilot's
